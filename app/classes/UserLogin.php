    ...
    public function addUser($data) {
        ...
        if ($this->doubleUser($username) == false) {
            ...
            if ($this->passwordCheck($pass, $conpass) == false) {
                $err_txt = "Password Doesn't match!";
                return $err_txt;
            } else {
                if ($this->doubleEmail($email) == false) {
                    ...
                    $sql = "INSERT INTO `users` (`username`, `email`, `password`, `role`, `fname`) VALUES (?, ?, ?, ?, ?)";
                    $stmt = $db->prepare($sql);
                    // Bind parameters
                    $stmt->bind_param("sssss", $username, $email, $pass, $role, $fname);
