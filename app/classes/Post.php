...
class Post
{
    public function addPost($data) {
        $cat_id = $data['cat_id'];
        $title = Helper::filter($data['title']);
        $content = $data['content']; // Assuming you want to filter the content
        $tag = Helper::filter($data['tag']); // Assuming you want to filter the tag
        $status = $data['status'];
        ...
        $sql = "INSERT INTO `blog` (`cat_id`, `title`, `content`, `tag`, `admin`, `status`, `image`, `rate`) VALUES (?, ?, ?, ?, ?, ?, ?, '1')";
        $stmt = $db->prepare($sql);

        if ($stmt) {
            // Bind parameters
            $stmt->bind_param("sssssss", $cat_id, $title, $content, $tag, $admin, $status, $image);
            ...
            $txt = "Error preparing statement: " . $db->error;
        }
    ...
    public function updatePost($data) {
        ...
        if ($_FILES['image']['name'] == NULL) {
            $sql = "UPDATE `blog` SET `cat_id` = ?, `title` = ?, `content` = ?, `tag` = ?, `admin` = ?, `status` = ? WHERE id = ?";
        } else {
            $image = $_FILES['image']['name'];
            $img_ext = pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION);
            $image = rand(1, 888888) . '.' . $img_ext;
            ...
            $sql = "UPDATE `blog` SET `cat_id` = ?, `title` = ?, `content` = ?, `tag` = ?, `admin` = ?, `image` = ?, `status` = ? WHERE id = ?";
            $upload = '../uploads/' . $image;
            move_uploaded_file($_FILES['image']['tmp_name'], $upload);
        ...
        if ($stmt) {
            // Bind parameters
            if ($_FILES['image']['name'] == NULL) {
                $stmt->bind_param("ssssssi", $cat_id, $title, $content, $tag, $admin, $status, $id);
            } else {
                $stmt->bind_param("sssssssi", $cat_id, $title, $content, $tag, $admin, $image, $status, $id);
            }
            ...
            $uptxt = "Error preparing statement: " . $db->error;
        }
